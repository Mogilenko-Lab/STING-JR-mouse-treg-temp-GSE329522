...
why. Over-representation gives a gene as many memberships as it has terms: the 386 enriched GO
BP terms for `WT_heat_up` hand out 3,621 gene-term memberships over a union of 159 genes, and 9
of the 199 genes sit in exactly one enriched term. Collapsed to the 35 similarity blocks it is
972 memberships with 21 genes in one block. Shares taken off that do not add up to the arm. The
...
386 enriched terms sit against a matched-null median of 242, 95th percentile 382 and maximum
588, permutation p 0.047. The 159 genes those terms cover sit against a null median of 147 and
95th percentile 158, permutation p **0.050**. A uniform null of the same size reaches a median
of 0 terms.

That coverage p is the number to hold onto. "GO BP places 159 of the 199 genes in an enriched
term" was the headline number here until this run, and it is now known to be about what a
random set of equally well-studied genes achieves. The other three arms behave differently:
`KO_heat_up` covers 165 against a null median of 130 (p 0.001), and both `Interaction` arms
clear their nulls comfortably because a 7-gene or 18-gene draw usually reaches nothing at all.
The arm this project cares about is the weakest of the four on both counts.
...
significance. The median term recurs in 10.3% of draws, 138 of 386 terms recur in more than 20%
and 25 in more than 50%. Recurrence is highest among the terms a reader would quote first:
chemotaxis 0.281, angiogenesis 0.366, blood vessel morphogenesis 0.471, positive regulation of
cell migration 0.586, positive regulation of cytokine production 0.628. A term at 0.6 is one
...
p_matched 0.101). Both are carried by inflammatory mediators. Every chaperone, folding and
...
`GO:0006954 inflammatory response` holds 688 genes in the with_iea universe, above the 500-gene
cap, so it never entered the test at all while claiming 43 of the arm's 199 genes, the largest
claim of any term in the panel. Under `no_iea` it falls under the cap, enters, and comes back
5th of 227 for `WT_heat_up` and 2nd for `KO_heat_up`. Both variants sit on the same row.
...
share 185 of the 386 with_iea BP terms (Jaccard 0.43) and only 4 of their top 10, so the
evidence filter moves the headline list. The same question asked in mouse space before the
ortholog map returns 456 BP terms sharing 314 with the human-space 386 (Jaccard 0.59).
...
The 386 GO BP terms enriched for WT_heat_up collapse to 35 blocks.
...
For WT_heat_up the 386 enriched GO BP terms sit against a
depth-matched null median of 242 and 95th percentile of 382,
permutation p 0.047 over 2000 replicates. The 159 arm genes in at
least one enriched term sit against a null median of 147 and 95th
percentile of 158, permutation p 0.05. A uniformly drawn null of the
...
Over the 386 GO BP terms enriched for WT_heat_up, a depth-matched
random draw of the same size reaches q below 0.05 for the median term
in 10.3% of 2000 replicates, for 138 terms in more than 20% and for
25 terms in more than 50%. Recurrence rises towards the head of the
list: angiogenesis 0.366, blood vessel morphogenesis 0.470, positive
regulation of cytokine production 0.627, chemotaxis 0.281, positive
...
159 of WT_heat_up's 199 genes sit in at least one enriched GO BP
term, 29 carry an annotation and sit in none, and 11 carry no BP
annotation at all (7 of those are visible in MF). A depth-matched
random draw of 188 genes covers a median of 147, so the permutation p
on the coverage number is 0.05.
...
|---|---|---|
| `go_enriched_terms.csv` | Every enriched term for every arm x ontology x IEA variant, 1,466 rows, with the clusterProfiler columns plus `term_size`, `fold_enrichment`, `simplify_kept`, the block assignment, and the null columns. | Filter on `arm`, `ontology` and `iea_variant` before reading anything. The null columns are populated for the primary combination (BP, with_iea) only and are NA elsewhere, because the null is computed for the primary ontology alone. `Count` is arm genes in the term, `term_size` the term's size in the 12,986-symbol background. |
