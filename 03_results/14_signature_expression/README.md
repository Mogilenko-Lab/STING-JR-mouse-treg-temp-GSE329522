...
one human ortholog, so n_genes_mouse - n_dropped_unmapped_total =
...
smaller whenever two mouse genes collapse onto one ortholog.

The drop total then splits, and the split is the point.
n_dropped_no_ortholog is the orthology source knowing the symbol and
having no human counterpart. n_dropped_stale_query_symbol is it not
being able to key the symbol at all, because this matrix carries an
older MGI vintage than the source expects — a vocabulary result that
used to be reported as the first kind. n_query_symbol_normalised
counts genes that arrived ONLY because their symbol was lifted to its
current form before the source was asked.

median_abs_t is 10.8/10.3 for the heat arms against 6.7/5.8 for the
interaction arm, the power gap between a 10-vs-10 contrast and a 1-df
interaction term. n_genes_plotted records the figure's cap. Tier: L3.
