...
set is stacked by fate: green = mapped 1:1, orange =
one-mouse->many-human (each human ortholog inherits the mouse t in
the ranked list; unioned in the binary set), grey = dropped (no human
ortholog). A contrast carried at two threshold gates gets one bar per
gate, with the gate named in square brackets under its tick. Bar
height = mouse-set size; '->N' above = resulting human-set size. A
red * marks > 50% dropped — a decimated signature to demote. Read it
as the sanity check that the frozen human sets are not hollow. Claim
tier: L3.
